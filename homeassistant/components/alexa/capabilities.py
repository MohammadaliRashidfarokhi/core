...
class AlexaThermostatController(AlexaCapability):
    ...
    def _get_thermostat_mode_property(self, name: str) -> dict[str, str] | str | None:
        """Handle the 'thermostatMode' property."""
    ...
    def _get_setpoint_property(self, name: str) -> dict[str, Any] | None:
        """Handle setpoint properties."""
        unit = self.hass.config.units.temperature_unit
...
class AlexaModeController(AlexaCapability):
    ...
    def _get_fan_preset_mode(self) -> str | None:
    ...
    def _get_remote_activity_mode(self) -> str | None:
    ...
    def _get_cover_position_mode(self) -> str | None:
    ...
    def _get_valve_position_state(self) -> str | None:
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        if self.instance == f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}":
            return self._fan_direction_resource()
        if self.instance == f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}":
            return self._fan_preset_mode_resource()
        if self.instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}":
            return self._humidifier_mode_resource()
        if self.instance == f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}":
            return self._water_heater_operation_mode_resource()
        if self.instance == f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}":
            return self._remote_activity_resource()
        if self.instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
            return self._cover_position_resource()
        if self.instance == f"{valve.DOMAIN}.state":
            return self._valve_position_resource()
        return {}

    def _fan_direction_resource(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _humidifier_mode_resource(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _water_heater_operation_mode_resource(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource([AlexaGlobalCatalog.SETTING_MODE], False)
        operation_modes = (
            self.entity.attributes.get(water_heater.ATTR_OPERATION_LIST) or []
        )
    ...
    def _remote_activity_resource(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource([AlexaGlobalCatalog.SETTING_MODE], False)
    ...
    def _cover_position_resource(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource(
    ...
    def _valve_position_resource(self) -> dict[str, list[dict[str, Any]]]:
        supported_features = self.entity.attributes.get(ATTR_SUPPORTED_FEATURES, 0)
        self._resource = AlexaModeResource(
...
class AlexaRangeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return capabilityResources object."""
        instance_map = {
            f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}": self._fan_percentage_resource,
            f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}": self._humidifier_humidity_resource,
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": self._cover_position_resource,
            f"{cover.DOMAIN}.tilt": self._cover_tilt_resource,
            f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}": self._input_number_value_resource,
            f"{number.DOMAIN}.{number.ATTR_VALUE}": self._number_value_resource,
    ...
    def _fan_percentage_resource(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _humidifier_humidity_resource(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaPresetResource(
    ...
    def _cover_tilt_resource(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaPresetResource(
    ...
    def _input_number_value_resource(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _number_value_resource(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _vacuum_fan_speed_resource(self) -> dict[str, list[dict[str, Any]]]:
