...
ALEXA_ATTR_TYPE = "@type"  # single source of truth for the JSON type key
    ...
    def serialize_labels(self, resources: list[str]) -> dict[str, list[dict[str, Any]]]:
        ...
        for label in resources:
            if label in AlexaGlobalCatalog.__dict__.values():
                label_dict = {ALEXA_ATTR_TYPE: "asset", "value": {"assetId": label}}
            else:
                label_dict = {
                    ALEXA_ATTR_TYPE: "text",
...
class AlexaSemantics:
    ...
    def add_states_to_value(self, states: list[str], value: Any) -> None:
        ...
        self._add_state_mapping(
            {ALEXA_ATTR_TYPE: self.STATES_TO_VALUE, "states": states, "value": value}
        )
    ...
    def add_states_to_range(
        ...
        self._add_state_mapping(
            {
                ALEXA_ATTR_TYPE: self.STATES_TO_RANGE,
                "states": states,
    ...
    def add_action_to_directive(
        ...
        self._add_action_mapping(
            {
                ALEXA_ATTR_TYPE: self.ACTIONS_TO_DIRECTIVE,
                "actions": actions,
